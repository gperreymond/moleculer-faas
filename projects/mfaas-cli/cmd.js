...
program
  ...
  .option('-n, --namespace <NAMESPACE>', 'Namespace the service will be deploy in kubernetes', 'moleculerfaas-fn')
  ...
  .action(async (cmd) => {
    const { env: ENV, name: APP_NAME, replicas: REPLICAS, namespace: NAMESPACE, ingressHost: INGRESS_HOST, imageRepository: IMAGE_REPOSITORY, imageTag: IMAGE_TAG } = cmd
    const broker = new ServiceBroker({
      metrics: false,
      logger: false
    })
    await broker.loadServices()
    await broker.start()
    await broker.call('Commander.Deploy', { ENV, APP_NAME, NAMESPACE, IMAGE_REPOSITORY, IMAGE_TAG, INGRESS_HOST, REPLICAS })
  })
